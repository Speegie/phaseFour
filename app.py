...
def adminHome():
    if request.method == 'POST':
        ...
        if request.form['but'] == 'processD':
            return redirect('/adminProcessDate')
...
def scheduleFlight():
    ...
    if request.method == 'POST':
        if request.form['btn_identifier'] == 'sortOne':
            ...
            return render_template('scheduleFlight.html', num=num, dateStr=dateStr, airline=airline, cost=cost, fromA=fromA, toA=toA, capacity=capacity, depTime=depTime, arrTime=arrTime, currDate=currDate)
    ...
    if request.method == 'POST':
        if request.form['btn_identifier'] == 'sortOne':
            resultValue = cur.execute(
                "SELECT * FROM Flight order by Airline_Name asc")
        ...
        if request.form['btn_identifier'] == 'sortTwo':
            resultValue = cur.execute(
                "SELECT * FROM Flight order by cast(Flight_Num as SIGNED) desc")
            viewFDetails = cur.fetchall()
            return render_template('removeFlight.html', viewFDetails=viewFDetails, currDate=currDate)
        if request.form['btn_identifier'] == 'sortThree':
            resultValue = cur.execute(
                "SELECT * FROM Flight order by cast(Flight_Date as DATE) asc")
            viewFDetails = cur.fetchall()
            return render_template('removeFlight.html', viewFDetails=viewFDetails, currDate=currDate)
        if request.form['btn_identifier'] == 'sortFour':
            resultValue = cur.execute(
                "SELECT * FROM Flight order by cost desc")
            viewFDetails = cur.fetchall()
            return render_template('removeFlight.html', viewFDetails=viewFDetails, currDate=currDate)
        if request.form['btn_identifier'] == 'sortFive':
            resultValue = cur.execute(
                "SELECT * FROM Flight order by capacity desc")
            viewFDetails = cur.fetchall()
            return render_template('removeFlight.html', viewFDetails=viewFDetails, currDate=currDate)
        ...
        if (request.form['btn_identifier'] == 'sortDate'):
            ...
            resultValue = cur.execute(
                "SELECT * FROM Flight where Flight_Date between '" + (datee) + "' and '" + datee2 + "'")
        ...
        if (request.form['btn_identifier'] == 'sortLine'):
            ...
            resultValue = cur.execute(
                "SELECT * FROM Flight where Airline_Name = '" + (aline) + "'")
        ...
        if (request.form['btn_identifier'] == 'sortNum'):
            ...
            resultValue = cur.execute(
                "SELECT * FROM Flight where Flight_Num = " + (numm))
...
def viewAirports():
    ...
    if request.method == 'POST':
        if request.form['btn_identifier'] == 'sortOne':
            resultValue = cur.execute(
                "SELECT * FROM view_airports order by Airport_Id asc")
            viewAirportDetails = cur.fetchall()
            return render_template('viewAirports.html', viewAirportDetails=viewAirportDetails)
        if request.form['btn_identifier'] == 'sortTwo':
            resultValue = cur.execute(
                "SELECT * FROM view_airports order by Airport_Name asc")
            viewAirportDetails = cur.fetchall()
            return render_template('viewAirports.html', viewAirportDetails=viewAirportDetails)
        if request.form['btn_identifier'] == 'sortThree':
            resultValue = cur.execute(
                "SELECT * FROM view_airports order by Time_Zone asc")
            viewAirportDetails = cur.fetchall()
            return render_template('viewAirports.html', viewAirportDetails=viewAirportDetails)
        if request.form['btn_identifier'] == 'sortFour':
            text = request.form['text']
            resultValue = cur.execute(
                "SELECT * FROM view_airports where Airport_Name like '%" + text + "%'")
            viewAirportDetails = cur.fetchall()
            return render_template('viewAirports.html', viewAirportDetails=viewAirportDetails)
        if request.form['btn_identifier'] == 'sortFive':
            time = request.form.get('time')
            resultValue = cur.execute(
                "SELECT * FROM view_airports where time_zone = '" + time + "'")
            viewAirportDetails = cur.fetchall()
            return render_template('viewAirports.html', viewAirportDetails=viewAirportDetails)
        if request.form['btn_identifier'] == 'back':
            return redirect('/adminHome')
        if request.form['btn_identifier'] == 'sortSix':
            resultValue = cur.execute(
                "SELECT * FROM view_airports order by total_arriving_flights desc")
            viewAirportDetails = cur.fetchall()
            return render_template('viewAirports.html', viewAirportDetails=viewAirportDetails)
        if request.form['btn_identifier'] == 'sortSev':
            resultValue = cur.execute(
                "SELECT * FROM view_airports order by total_departing_flights desc")
            viewAirportDetails = cur.fetchall()
            return render_template('viewAirports.html', viewAirportDetails=viewAirportDetails)
        if request.form['btn_identifier'] == 'sortEight':
            resultValue = cur.execute(
                "SELECT * FROM view_airports order by avg_departing_flight_cost desc")
            viewAirportDetails = cur.fetchall()
            return render_template('viewAirports.html', viewAirportDetails=viewAirportDetails)
...
def viewCustomers():
    ...
    if request.method == 'POST':
        if request.form['btn_identifier'] == 'sortOne':
            resultValue = cur.execute(
                "SELECT * FROM view_customers order by customer_name asc")
            viewCustDetails = cur.fetchall()
            return render_template('viewCustomers.html', viewCustDetails=viewCustDetails)
        if request.form['btn_identifier'] == 'sortTwo':
            resultValue = cur.execute(
                "SELECT * FROM view_customers order by avg_rating desc")
            viewCustDetails = cur.fetchall()
            return render_template('viewCustomers.html', viewCustDetails=viewCustDetails)
        if request.form['btn_identifier'] == 'sortThree':
            resultValue = cur.execute(
                "SELECT * FROM view_customers order by location asc")
            viewCustDetails = cur.fetchall()
            return render_template('viewCustomers.html', viewCustDetails=viewCustDetails)
        if request.form['btn_identifier'] == 'sortFour':
            text = request.form['text']
            resultValue = cur.execute(
                "SELECT * FROM view_customers where customer_name like '%" + text + "%'")
            viewCustDetails = cur.fetchall()
            return render_template('viewCustomers.html', viewCustDetails=viewCustDetails)
        if request.form['btn_identifier'] == 'sortSix':
            resultValue = cur.execute(
                "SELECT * FROM view_customers order by is_owner desc")
            viewCustDetails = cur.fetchall()
            return render_template('viewCustomers.html', viewCustDetails=viewCustDetails)
        if request.form['btn_identifier'] == 'sortSev':
            resultValue = cur.execute(
                "SELECT * FROM view_customers order by total_seats_purchased desc")
            viewCustDetails = cur.fetchall()
            return render_template('viewCustomers.html', viewCustDetails=viewCustDetails)
        if request.form['btn_identifier'] == 'back':
            return redirect('/adminHome')
...
def viewFlights():
    ...
    if request.method == 'POST':
        if request.form['btn_identifier'] == 'sortOne':
            resultValue = cur.execute(
                "SELECT * FROM view_flight order by flight_id desc")
            viewFlightDetails = cur.fetchall()
            return render_template('viewFlights.html', viewFlightDetails=viewFlightDetails)
        if request.form['btn_identifier'] == 'sortTwo':
            resultValue = cur.execute(
                "SELECT * FROM view_flight order by airline asc")
            viewFlightDetails = cur.fetchall()
            return render_template('viewFlights.html', viewFlightDetails=viewFlightDetails)
        if request.form['btn_identifier'] == 'sortThree':
            resultValue = cur.execute(
                "SELECT * FROM view_flight order by destination asc")
            viewFlightDetails = cur.fetchall()
            return render_template('viewFlights.html', viewFlightDetails=viewFlightDetails)
        if request.form['btn_identifier'] == 'sortFour':
            text1 = request.form.get('text')
            resultValue = cur.execute(
                "SELECT * FROM view_flight where num_empty_seats < " + text1)
            viewFlightDetails = cur.fetchall()
            return render_template('viewFlights.html', viewFlightDetails=viewFlightDetails)
        if request.form['btn_identifier'] == 'sortEight':
            resultValue = cur.execute(
                "SELECT * FROM view_flight where num_empty_seats < 100000")
            viewFlightDetails = cur.fetchall()
            return render_template('viewFlights.html', viewFlightDetails=viewFlightDetails)
        if request.form['btn_identifier'] == 'sortFive':
            resultValue = cur.execute(
                "SELECT * FROM view_flight order by flight_date desc")
            viewFlightDetails = cur.fetchall()
            return render_template('viewFlights.html', viewFlightDetails=viewFlightDetails)
        if request.form['btn_identifier'] == 'sortSix':
            resultValue = cur.execute(
                "SELECT * FROM view_flight order by seat_cost desc")
            viewFlightDetails = cur.fetchall()
            return render_template('viewFlights.html', viewFlightDetails=viewFlightDetails)
        if request.form['btn_identifier'] == 'sortSev':
            resultValue = cur.execute(
                "SELECT * FROM view_flight order by total_spent desc")
            viewFlightDetails = cur.fetchall()
            return render_template('viewFlights.html', viewFlightDetails=viewFlightDetails)
        if request.form['btn_identifier'] == 'back':
            return redirect('/adminHome')
...
def viewProperties():
    ...
    if request.method == 'POST':
        ...
        if (request.form['btn_identifier'] == 'sortFour'):
            ...
            resultValue = cur.execute(
                "SELECT * FROM view_properties where capacity > " + (text1) + " and capacity < " + text2)
        ...
        if request.form['btn_identifier'] == 'sortEight':
            ...
            text2 = "infinity"
            resultValue = cur.execute(
                "SELECT * FROM view_properties where capacity < 100000")
            viewPropDetails = cur.fetchall()
            return render_template('viewProperties.html', viewPropDetails=viewPropDetails, text1=text1, text2=text2, status=status)
        if request.form['btn_identifier'] == 'sortFive':
            resultValue = cur.execute(
                "SELECT * FROM view_properties order by average_rating_score desc")
            viewPropDetails = cur.fetchall()
            return render_template('viewProperties.html', viewPropDetails=viewPropDetails, status=status)
        if request.form['btn_identifier'] == 'sortSix':
            resultValue = cur.execute(
                "SELECT * FROM view_properties order by capacity desc")
            viewPropDetails = cur.fetchall()
            return render_template('viewProperties.html', viewPropDetails=viewPropDetails, status=status)
        if request.form['btn_identifier'] == 'sortSev':
            resultValue = cur.execute(
                "SELECT * FROM view_properties order by cost_per_night desc")
            viewPropDetails = cur.fetchall()
            return render_template('viewProperties.html', viewPropDetails=viewPropDetails, status=status)
        if request.form['btn_identifier'] == 'back':
            return redirect('/custHome')
...
def viewIProperties():
    ...
    if request.method == 'POST':
        ...
        if (request.form['btn_identifier'] == 'ownEmail'):
            ...
            cur.execute("call view_individual_property_reservations('{}', '{}');".format(
                str(text2), str(text1)))
            mysql.connection.commit()
    ...
    if request.method == 'GET':
        resultValue = cur.execute(
            "call view_individual_property_reservations('{}', '{}');".format("*", "*"))
        mysql.connection.commit()
        viewPropDetails = cur.fetchall()
...
def viewOwners():
    ...
    if request.method == 'POST':
        if request.form['btn_identifier'] == 'sortOne':
            resultValue = cur.execute(
                "SELECT * FROM view_owners order by avg_rating desc")
            viewOwnersDetails = cur.fetchall()
            return render_template('viewOwners.html', viewOwnersDetails=viewOwnersDetails)
        if request.form['btn_identifier'] == 'sortTwo':
            resultValue = cur.execute(
                "SELECT * FROM view_owners order by num_properties_owned desc")
            viewOwnersDetails = cur.fetchall()
            return render_template('viewOwners.html', viewOwnersDetails=viewOwnersDetails)
        if request.form['btn_identifier'] == 'sortThree':
            resultValue = cur.execute(
                "SELECT * FROM view_owners order by avg_property_rating desc")
            viewOwnersDetails = cur.fetchall()
            return render_template('viewOwners.html', viewOwnersDetails=viewOwnersDetails)
        if request.form['btn_identifier'] == 'sortFour':
            text = request.form['text']
            resultValue = cur.execute(
                "SELECT * FROM view_owners where owner_name like '%" + text + "%'")
            viewOwnersDetails = cur.fetchall()
            return render_template('viewOwners.html', viewOwnersDetails=viewOwnersDetails)
        if request.form['btn_identifier'] == 'back':
            return redirect('/adminHome')
...
def ownerRemoveProperty():
    if request.method == 'POST':
        ...
        if request.form['btn_identifier'] == 'removeProperty':
            ...
            currentDate = "2021-10-15"
            cur = mysql.connection.cursor()
            cur.execute("call remove_property('{}', '{}', '{}');".format(
                name, email, currentDate))
            mysql.connection.commit()

            cur = mysql.connection.cursor()
            cur.execute("SELECT Property_Name, Descr, Capacity, Cost, (SELECT CONCAT(Street, ', ', City, ', ', State, ' ', Zip)) as Address FROM property where Owner_Email like '%" + email + "%'")
    ...
    if request.method == 'POST':
        ...
        if request.form['btn_identifier'] == 'submit':
            ...
            cur.execute("select Start_Date, cEmail, pName, Address, Score from owners_rate_customers right join (select pName, oEmail, cEmail, Start_Date, (SELECT CONCAT(Street, ', ', City, ', ', State, ' ', Zip)) as Address from property right join (select Property_Name as pName, Owner_Email as oEmail, Customer as cEmail, Start_Date, End_Date from reserve where Owner_Email = '" +
                        email + "') as outerOne on Owner_Email = oEmail and Property_Name = pName where oEmail = '" + email + "') as outerTwo on Customer = cEmail;")
            ...
            for i in range(len(newInput)):
                if newInput[i] != 'None' and int(newInput[i]) != ratingDetails[i][4]:
                    ...
                    cur.execute("call owner_rates_customer('{}', '{}', {}, '{}');".format(
                        email, ratingDetails[i][1], newInput[i], currentDate))
                    mysql.connection.commit()
...
def deleteOwnerAccount():
    if request.method == 'POST':
        ...
        if request.form['btn_identifier'] == 'deleteAccount':
            cur = mysql.connection.cursor()
            # .format(ownerEmail))
            cur.execute("call remove_owner('" + email + "');")
            mysql.connection.commit()
...
def viewAirlines():
    ...
    if request.method == 'POST':
        if request.form['btn_identifier'] == 'sortOne':
            resultValue = cur.execute(
                "SELECT * FROM view_airlines order by rating desc")
            viewAirlinesDetails = cur.fetchall()
            return render_template('viewAirlines.html', viewAirlinesDetails=viewAirlinesDetails)
        if request.form['btn_identifier'] == 'sortTwo':
            resultValue = cur.execute(
                "SELECT * FROM view_airlines order by total_flights desc")
            viewAirlinesDetails = cur.fetchall()
            return render_template('viewAirlines.html', viewAirlinesDetails=viewAirlinesDetails)
        if request.form['btn_identifier'] == 'sortThree':
            resultValue = cur.execute(
                "SELECT * FROM view_airlines order by min_flight_cost desc")
            viewAirlinesDetails = cur.fetchall()
            return render_template('viewAirlines.html', viewAirlinesDetails=viewAirlinesDetails)
        if request.form['btn_identifier'] == 'sortFour':
            text = request.form['text']
            resultValue = cur.execute(
                "SELECT * FROM view_airlines   where airline_name like '%" + text + "%'")
            viewAirlinesDetails = cur.fetchall()
            return render_template('viewAirlines.html', viewAirlinesDetails=viewAirlinesDetails)
        if request.form['btn_identifier'] == 'back':
            return redirect('/adminHome')
...
def bookFlight():
    ...
    if request.method == 'POST':
        if request.form['btn_identifier'] == 'sortByAirline':
            resultValue = cur.execute(
                'SELECT Airline, flight_id, num_empty_seats FROM view_flight order by airline')
        ...
        if request.form['btn_identifier'] == 'sortByFlightNum':
            resultValue = cur.execute(
                'SELECT Airline, flight_id, num_empty_seats FROM view_flight order by CAST(flight_id as UNSIGNED) asc')
            bookFlightDetails = cur.fetchall()
            return render_template('bookFlight.html', bookFlightDetails=bookFlightDetails)
        ...
        if request.form['btn_identifier'] == 'submitFlight':
            ...
            cur.execute("call book_flight('{}', '{}','{}',{},'{}');".format(
                email, flight_id, airline, numSeats, currentDate))
            mysql.connection.commit()
            resultValue = cur.execute(
                'SELECT Airline, flight_id, num_empty_seats FROM view_flight order by airline')
            bookFlightDetails = cur.fetchall()
...
def cancelFlight():
    ...
    if request.method == 'POST':
        if request.form['btn_identifier'] == 'sortByAirline':
            resultValue = cur.execute(
                "SELECT Airline_Name, Flight_Num FROM book where Customer = '{}' order by Airline_name".format(email))
            cancelFlightDetails = cur.fetchall()
            return render_template('cancelFlight.html', cancelFlightDetails=cancelFlightDetails)

        if request.form['btn_identifier'] == 'sortByFlightNum':
            resultValue = cur.execute(
                "SELECT Airline_Name, Flight_Num FROM book where Customer = '{}' order by CAST(flight_num as UNSIGNED) asc".format(email))
            cancelFlightDetails = cur.fetchall()
            return render_template('cancelFlight.html', cancelFlightDetails=cancelFlightDetails)
        ...
        if request.form['btn_identifier'] == 'searchFlight':
            ...
            if (airline) != "":
                resultValue = cur.execute(
                    "SELECT Airline_Name, Flight_Num FROM book where Customer='{}' AND Airline_Name like '%{}%'".format(email, airline))
                cancelFlightDetails = cur.fetchall()
                return render_template('cancelFlight.html', cancelFlightDetails=cancelFlightDetails)
            elif flight_num != "":
                resultValue = cur.execute(
                    "SELECT Airline_Name, Flight_Num FROM book where Customer='{}' AND Flight_Num ='{}'".format(email, flight_num))
                cancelFlightDetails = cur.fetchall()
                return render_template('cancelFlight.html', cancelFlightDetails=cancelFlightDetails)
            else:
                resultValue = cur.execute(
                    'SELECT Airline_Name, Flight_Num FROM book where Customer = ' + "'" + email + "'")
                cancelFlightDetails = cur.fetchall()
                return render_template('cancelFlight.html', cancelFlightDetails=cancelFlightDetails)
        ...
        if request.form['btn_identifier'] == "submitCancelFlight":
            ...
            resultValue = cur.execute("call cancel_flight_booking('{}','{}','{}','{}');".format(
                email, flight_num, airline, currentDate))
            mysql.connection.commit()
            resultValue = cur.execute(
                'SELECT Airline_Name, Flight_Num FROM book where Customer = "{}" and Was_Cancelled=0'.format(email))
            cancelFlightDetails = cur.fetchall()
            return render_template('cancelFlight.html', cancelFlightDetails=cancelFlightDetails)
...
def reserveProperty():
    ...
    if request.method == 'POST':
        if request.form['btn_identifier'] == 'sortByName':
            resultValue = cur.execute(
                'SELECT (reserve.Property_Name), (reserve.Owner_Email), Capacity from reserve natural join property order by reserve.Property_Name')
        ...
        if request.form['btn_identifier'] == 'sortByEmail':
            resultValue = cur.execute(
                'SELECT (reserve.Property_Name), (reserve.Owner_Email), Capacity from reserve natural join property order by reserve.Owner_Email')
            reservePropertyDetails = cur.fetchall()
            return render_template('reserveProperty.html', reservePropertyDetails=reservePropertyDetails)

        if request.form['btn_identifier'] == 'sortByCapacity':
            resultValue = cur.execute(
                'SELECT (reserve.Property_Name), (reserve.Owner_Email), Capacity from reserve natural join property order by Capacity')
            reservePropertyDetails = cur.fetchall()
            return render_template('reserveProperty.html', reservePropertyDetails=reservePropertyDetails)
            ...
            for i in range(len(newInput)):
                if newInput[i] != 'None':
                    cur.execute("call customer_review_property('{}', '{}', '{}', '{}', '{}', '{}');".format(
                        reviewDetails[i][1], reviewDetails[i][2], customerEmail, description, score, currentDate))
                    mysql.connection.commit()
    ...
    if request.method == 'POST':
        ...
        if request.form['btn_identifier'] == 'submit':
            ...
            for i in range(len(newInput)):
                if newInput[i] != 'None':
                    newInput[i] = int(newInput[i])
                    cur.execute("call customer_rates_owner('{}', '{}', {}, '{}');".format(
                        customerEmail, ratingDetails[i][1], newInput[i], currentDate))
                    mysql.connection.commit()
